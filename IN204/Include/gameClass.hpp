...
enum commandes {
  ...
  ping,
  byebye,
}; // commandes possibles recues par le Board// pause n'est pas vraiment recue,
   // mais elle est enregistree. il faut deux commandes d'un m�me type suivies
   // d'une pause pour les envoyer toutes les deux

class Piece;
class Game;

class Board // le point (0,0) est en haut a gauche// correspond � un 'ecran' ie
            // le jeu d'un joueur
{
public:
  int nr_joueur;
  ...
  int HEIGHT; // en nombre de cases (pour pas voir de probleme d'arrondi en cas
              // de divisions...)
  int WIDTH;  // en nombre de cases (pour pas voir de probleme d'arrondi en cas
              // de divisions...)
  enum color *MATRIX; // matrice de cases color�es
  Piece *current_piece;
  std::list<enum type>
      pieces_suivantes; // ainsi, on peut faire pop_front en
                        // temps constant en evitant les copies;
  std::list<enum commandes> commandes_recues;

  Board(int step, int hauteur, int largeur, enum type pieceT,
        int numero_joueur);
  ~Board();
  ...
  int get_matrix_size();
  void continuer(); // fait avancer le plateau de jeu: fait tomber la piece, la
                    // fait tourner, et bouger; Elle est appel�e de l'ext�rieur
                    // uniquement aux bonnes �tapes
...
public:
  ...
  std::list<enum type>
      pieces_suivantes; // contient les pieces a venir /// deux pour pouvoir
                        // allouer en asynchrone (event.) // les passe aux Board
                        // chaque fois que l'un d'entre eux en demande une.
...
public:
  ...
  std::vector<int> matrice; // petite matrice 3*3, 2*2 ou 4*4 contenant la
                            // forme// 1 = occup�e, 0= non occup�e
  int taille_matrice;       // taille de la matrice
  int rotation; // appartient � |[0, 3]| la rotation r�elle en sera un multiple:
                // aide � rep�rer la forme dans le plan

  Piece();
  Piece(enum type typeP, enum color colorP, int x_premiere_case,
        int y_premiere_case);
  ~Piece();
